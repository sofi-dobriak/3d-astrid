...
class FilterModel extends EventEmitter {
  ...
  /* Ручна зміна фільтру - зроблено для відповідей з чатіка ai-sales-agent */
  changeFilterProgramatically(keyArgs, value) {
    console.log(this, key, value);
    const key = keyArgs === 'bedrooms' ? 'rooms' : keyArgs;
    if (!this.configProject[key]) {
      console.warn(`Filter with key "${key}" does not exist`);
      return;
    }
    if (typeof value === 'object' && this.configProject[key].type === 'range') {
      if (value.min === undefined || value.max === undefined) {
        console.warn(`For range filter, value should be an object with "min" and "max" properties`);
        this.configProject[key].elem.update({ from: value.from, to: value.to });
      } else {
        this.configProject[key].elem.update({ from: value.min, to: value.max });
      }
    }
    if (this.configProject[key].type === 'checkbox') {
      if (typeof value === 'string') {
        this.configProject[key].elem.forEach(el => {
          el.checked = el.dataset[key] == value;
        });
      }
      if (Array.isArray(value)) {
        value.forEach(val => {
          this.configProject[key].elem.forEach(el => { 
            console.log(el, el.dataset[key], val);
